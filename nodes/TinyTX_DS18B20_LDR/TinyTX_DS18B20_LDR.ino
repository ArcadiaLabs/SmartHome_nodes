...
#define ONE_WIRE_BUS 9   // DS18B20 Temperature sensor is connected on D10/ATtiny pin 13
...
void loop() {
  ...
  digitalWrite(LDR_POWER, HIGH); // turn LDR on
  delay(10);
  bitClear(PRR, PRADC); ADCSRA |= bit(ADEN);
  tinytx.light = 1024-analogRead(LDR_BUS);
  ADCSRA &= ~ bit(ADEN); bitSet(PRR, PRADC); // Disable the ADC to save power
  digitalWrite(LDR_POWER, LOW); // turn LDR off
